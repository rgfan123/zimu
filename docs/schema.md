...
- 当前权威快照共 72 张业务表、4 个分析视图和 2 个操作视图。
...
2. `information_schema` 实测 72 张 `app` 基础表、2 个 `app` 操作视图和 4 个 `analytics` 分析视图。
...
5. `SchemaSnapshotMigrationEquivalenceTest`（Testcontainers，`mvn test` 默认阶段运行）：分别用
   `docs/schema.sql` 与 Flyway 全链（V1..V68）建库，比对 12 类结构事实（见 §1 引言），不等价即失败。
