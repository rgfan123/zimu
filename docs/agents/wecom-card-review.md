...
> **2026-08-26 补充（#127/#147）**：企微当前 `task_id` 不接受冒号。现行投递格式为
> `{domain}_{id}_{marker}_{128-bit授权引用}`；实体与版本只从持久化 SENT 投递记录取得，
> 不信任回调字符串。普通业务卡仍以 3 个按钮为实用上限；Business Follow-up 的四个互斥
> 决定是经父规格明确批准的唯一 4 按钮例外，且资料不完整时必须移除“确认”。需要填写理由
> 或补充复杂字段的动作必须跳转认证后台，不能用零参数回调代替。
> #147 的草稿审批卡可投递到指定人单聊或明确配置的内部群；只有“确认”是企微回调。
> 重做/补充/暂停深链固定 `expected_draft_version`，128-bit 随机能力放在 URL fragment（不进入
