...
| 手工建单 | `/orders/manual-create` | **V100 新增可见叶子，不受控**（无 `requiresModule`；路由与 routeElements 常开） | 归属「订单与发货」：运营柜台直录 MANUAL 订单并当场路由出发货单，是真实触发京东发货的入口，能力随核心订单域常开、无独立接通判据（不像客户中心/原料库存那样依赖外部网关就绪）。可见叶子计入「订单与发货」板块，见第 3 节。 |
| 客户中心 | `/kehuzx-portal` | **票 04 同批：受运行期模块清单控制**（`requiresModule: 'customer-center'`，`external: '/kehuzx/'` 外链直开，不注册本应用路由） | 归属「我的工作台」，与「客户跟进」共用同一接通判据（`customer-center` 模块 = kehuzx 只读网关就绪）：接通才进菜单，未接通只是不显示；`path` 取 `/kehuzx-portal` 只作菜单标识，避免与反代前缀 `/kehuzx/` 撞出「点菜单却被本应用兜底路由接走」的歧义，因此不计入可路由叶子（`routableNavigationLeaves` 排除 external 节点）。 |
...
| 指标 | 收敛前 | #98 收敛后 | **Issue #104 换壳后** | **UIUX-02 合并后** | **UIUX-10 收敛后** | **票 04 后** | **票 05 后** | **票 06 后** | **V100 后（当前实测，2026-09-01）** |
|---|---|---|---|---|---|---|---|---|---|
| 一级板块（顶级菜单项） | 10 | 10 | **11**（新增「我的工作台」排最前，spec #103 D6） | **11**（不变） | **11**（不变） | **10** | **10**（不变） | **10**（不变） | **10**（不变） |
| 可见入口（可见叶子，含外链） | 31 | 30 | **32**（shipping/recon 升为可见 +2；`/demo/order` 隐藏 −1；#110 新增 `/workbench/procurement` +1） | **28**（订单中心三预设并入「全部订单」−3；采购协同仍可见） | **23**（UIUX-10：采购去重 −1、Agent 中心 −3、京东工具 6→1 −5） | **26 / 25** | **26 / 25**（不变：票 05 只加隐藏叶子） | **27（全部模块开放）/ 25（两个受控模块都未接通）**（+`/inventory/raw-materials` 可见受控叶子） | **29（全部模块开放）/ 26（两个受控模块都未接通）**（V100 新增 `/orders/manual-create` 不受控 +1、`/kehuzx-portal` 受控外链 +1） |
| 可路由叶子（不含外链，全部生产路由） | 38 | 42 | **43**（#110 新增采购工作台路由；既有 URL 零删改） | **46**（自 #104 后新增 agents/上传等路由） | **47**（+`/system/jd-tools` 新入口；既有 URL 零删改） | **50** | **51**（+`/system/mcp-exposure` 隐藏叶子） | **52**（+`/inventory/raw-materials`；既有 URL 零删改） | **53**（+`/orders/manual-create`；`/kehuzx-portal` 是 external 节点，不计入可路由叶子） |
| 作业中心可见叶子 | 8 | 6 | **5**（复核收件箱移入我的工作台） | **5**（不变） | **4**（采购协同降级，采购唯一入口在我的工作台） | **2** | **2**（渠道与文件：渠道消息 / 文件作业） | **2**（不变） | **2**（不变） |
| 我的工作台可见叶子 | — | — | **4**（今日发货/复核收件箱/采购工作台/对账工作台） | **4**（不变） | **4**（采购工作台更名「采购」） | **5 / 4** | **5（客户中心已接通）/ 4（未接通）** | **5 / 4**（不变） | **6（两个受控入口都接通）/ 4（都未接通）**（+`/kehuzx-portal`；**已顶 ≤6 硬上限，板块内不能再加可见叶子**） |
| 订单与发货可见叶子 | — | — | — | — | — | — | — | — | **4**（新增，+`/orders/manual-create`；全部订单/手工建单/发货记录/履约任务） |
| 商品与主数据可见叶子 | — | — | — | — | — | **4** | **4** | **5（原料库存已接通）/ 4（未接通）** | **5 / 4**（不变） |

各一级板块可见子项（V100 后实测，按「全部模块开放」口径判上限，均 ≤ 6）：我的工作台 6/4
（客户跟进 + 客户中心两个受控入口）、订单与发货 4（新增手工建单）、渠道与文件 2、Agent 中心 3、
商品与主数据 5/4（原料库存受控）、系统与接入 6（**已满，不能再加可见叶子**——票 05 的 MCP 开放面
因此走降级，见第 2 节收敛清单）。**我的工作台在全部模块开放时同样已到 6 的硬上限**——后续再给
该板块加可见叶子（无论是否受控）前必须先降级或合并既有入口，不能直接追加第 7 项。
