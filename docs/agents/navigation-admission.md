...
| 可见入口（可见叶子，含外链） | 31 | 30 | **32**（shipping/recon 升为可见 +2；`/demo/order` 隐藏 −1；#110 新增 `/workbench/procurement` +1） |
| 可路由叶子（不含外链，全部生产路由） | 38 | 42 | **43**（#110 新增采购工作台路由；既有 URL 零删改） |
| 作业中心可见叶子 | 8 | 6 | **5**（复核收件箱移入我的工作台；渠道消息/履约任务/采购协同/文件作业/发货记录） |
| 我的工作台可见叶子 | — | — | **4**（今日发货工作台/复核收件箱/采购工作台/对账工作台——spec D6 目标形态达成） |
