...
-- BEGIN V61__wecom_chat_profiles.sql
-- 会话档案化：备注名（协议不下发群名，只能人起）与服务 Agent；reply_mode 补缺省。
...
-- BEGIN V63__product_archive_sheets.sql
...
--   AI「线下供货成本/份」= 成本（权威成本列，按份 = 500g 单袋口径）；
--   AJ「售价」          = 不含运费的售价。
-- AJ **不**入 retail_price：口径与库内京东整售零售价不同，只留在本档案里。
-- AI 按份价并入 purchase_price 走权威目录 manifest 治理路线，不由本表裸写。
--
...
-- BEGIN V65__business_followups.sql
...
-- END V65__business_followups.sql

-- BEGIN V66__kehuzx_followup_drafts.sql
...
-- END V66__kehuzx_followup_drafts.sql

-- BEGIN V67__business_followup_approvals.sql
...
-- rows can be upgraded to a fresh opaque authorization capability.
...
-- END V67__business_followup_approvals.sql

-- BEGIN V68__business_followup_assignments.sql
...
-- END V68__business_followup_assignments.sql

-- BEGIN V71__business_followup_execution_intent.sql
ALTER TABLE app.business_followups
...
-- END V71__business_followup_execution_intent.sql

-- BEGIN V72__source_order_intake_jobs.sql
...
-- BEGIN V73__source_sync_auto_states.sql
...
-- V78–V82 为在途交付线预留后未用，编号跳号直发 V83。

-- BEGIN V83__scheduled_pull_runs.sql
-- 定时平台拉取 + 自动发货的运行记录。
--
-- 存在的理由有三个，缺一个都不够：
...
-- END V83__scheduled_pull_runs.sql
...
-- BEGIN V85__scheduled_pull_per_channel.sql
...
-- END V85__scheduled_pull_per_channel.sql
...
-- BEGIN V88__order_line_source_sku_ref.sql
...
-- END V88__order_line_source_sku_ref.sql

-- BEGIN V89__freeze_allocated_order_line_source_sku_ref.sql
...
-- END V89__freeze_allocated_order_line_source_sku_ref.sql

-- BEGIN V90__structured_product_sku_identity.sql
...
-- END V90__structured_product_sku_identity.sql

-- BEGIN V91__sku_data_quality_flags.sql
...
FOR EACH ROW EXECUTE FUNCTION app.set_updated_at();
-- END V91__sku_data_quality_flags.sql

-- BEGIN V92__sku_readiness_catalog_lock.sql
...
FOR EACH ROW EXECUTE FUNCTION app.lock_sku_readiness_catalog_write();
-- END V92__sku_readiness_catalog_lock.sql

-- BEGIN V93__trusted_source_template_profiles.sql
...
-- END V93__trusted_source_template_profiles.sql

-- BEGIN V94__serialize_sku_catalog_writes_before_rows.sql
...
BEFORE INSERT OR UPDATE OR DELETE ON app.source_channel_bundles
FOR EACH STATEMENT EXECUTE FUNCTION app.lock_sku_readiness_catalog_write();
-- END V94__serialize_sku_catalog_writes_before_rows.sql

-- BEGIN V95__index_trusted_source_template_batch.sql
...
-- END V95__index_trusted_source_template_batch.sql
...
FOR EACH STATEMENT EXECUTE FUNCTION app.assert_active_sku_effective_barcode_unique();
-- END V96__enforce_active_sku_barcode_uniqueness.sql

-- BEGIN V99__integer_goods_quantities.sql
...
-- BEGIN V101__bundle_composer_agent.sql
