...
-- 空库权威快照：必须与 Flyway 全链（V1..V34）结构等价，由
-- SchemaSnapshotMigrationEquivalenceTest 保证；schema 变更需同时落新迁移与本文件（见 docs/schema.md §11）。
...
CREATE TABLE app.source_return_exports (
    ...
    generated_by        VARCHAR(128) NOT NULL,
    generated_at        TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP,
    generated_from_tracking_batch_id BIGINT REFERENCES app.import_batches(id) ON DELETE RESTRICT,
...
CREATE TABLE app.channel_messages (
    ...
    sender_identity_type VARCHAR(16) NOT NULL DEFAULT 'EMPLOYEE'
                        CHECK (sender_identity_type IN ('EMPLOYEE', 'CUSTOMER')),
    sender_access_type  VARCHAR(64),
...
CREATE UNIQUE INDEX uq_wecom_events_type_msgid
...
CREATE TABLE app.agent_runs (
    ...
    created_at              TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP,
    -- V33 增列：PREVIEW 行不进评测基线；intent/provider 供意图链路直接追溯
...
-- ---------------------------------------------------------------------------
-- Agent 平台定义与评测用例（V33）：append-only 全快照 + 数据化评测
-- ---------------------------------------------------------------------------

CREATE TABLE app.agent_definitions (
    id                  BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    agent_slug          VARCHAR(64)  NOT NULL CHECK (agent_slug ~ '^[a-z][a-z0-9-]{0,63}$'),
    ...
    guard_exemptions    TEXT[]       NOT NULL DEFAULT '{}',
    ...
    CHECK (array_position(tool_whitelist, NULL) IS NULL),
    CHECK (guard_exemptions <@ ARRAY['PII']::TEXT[]),
    CHECK (array_position(guard_exemptions, NULL) IS NULL),
...
CREATE UNIQUE INDEX uq_agent_definitions_active_slug
    ON app.agent_definitions (agent_slug) WHERE status = 'active';

CREATE INDEX idx_agent_definitions_slug_version
    ON app.agent_definitions (agent_slug, version DESC);

CREATE TABLE app.agent_eval_cases (
    id                  BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    agent_slug          VARCHAR(64)  NOT NULL CHECK (agent_slug ~ '^[a-z][a-z0-9-]{0,63}$'),
    agent_version       INTEGER      NOT NULL CHECK (agent_version > 0),
    eval_set_version    VARCHAR(64)  NOT NULL CHECK (btrim(eval_set_version) <> ''),
    case_key            VARCHAR(128) NOT NULL CHECK (btrim(case_key) <> ''),
    ...
    confirmed_at        TIMESTAMPTZ,
    created_at          TIMESTAMPTZ  NOT NULL DEFAULT CURRENT_TIMESTAMP,
    UNIQUE (agent_slug, agent_version, case_key),
...
CREATE INDEX idx_agent_eval_cases_slug_version_kind
...
CREATE INDEX idx_source_return_exports_tracking_batch
