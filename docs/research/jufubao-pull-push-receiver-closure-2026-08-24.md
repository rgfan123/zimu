...
2. 抽取一个共享 `JufubaoSessionAdapter`：负责 portal seed、登录、Cookie/CSRF、公共业务头与超时；只读请求遇 401 最多重登并重试一次，外部写遇 401 不重放并转 `RECONCILIATION_REQUIRED`。pull 与 push 只通过该适配器发请求，凭据仅从配置注入。
...
- [ ] 受控 HTTP 测试覆盖一次登录会话被 pull/push 共享、CSRF/header、只读 401 最多重登录一次、写请求 401/3xx 不重放、分页和敏感信息不进日志。
