...
BEGIN
    -- 与 V96 使用同一 fail-fast 协议：不等待已在途的目录写，也不给尚未进入
    -- V94 trigger 的表写留下反向锁序窗口。
    IF NOT pg_try_advisory_xact_lock(756426269156::BIGINT) THEN
        RAISE EXCEPTION 'V97 requires a quiescent SKU catalog; retry after active catalog writes finish'
            USING ERRCODE='55P03';
    END IF;
    BEGIN
        LOCK TABLE
    ...
    IF NOT pg_try_advisory_xact_lock(756426269157::BIGINT) THEN
        RAISE EXCEPTION 'V97 requires quiescent WECOM draft candidate creation; retry after active draft creation finishes'
            USING ERRCODE='55P03';
    END IF;
    BEGIN
        LOCK TABLE
        ...
        IN SHARE MODE NOWAIT;
    EXCEPTION WHEN lock_not_available THEN
        RAISE EXCEPTION 'V97 requires quiescent WECOM draft tables; retry after active draft writes finish'
            USING ERRCODE='55P03';
    END;
