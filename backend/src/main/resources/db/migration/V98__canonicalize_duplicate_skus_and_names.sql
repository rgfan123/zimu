...
BEGIN
    IF NOT pg_try_advisory_xact_lock(756426269156::BIGINT) THEN
        RAISE EXCEPTION 'V98 requires a quiescent SKU catalog; retry after active catalog writes finish'
            USING ERRCODE='55P03';
    END IF;
    BEGIN
        LOCK TABLE
    ...
    BEGIN
        ...
        IN SHARE MODE NOWAIT;
    EXCEPTION WHEN lock_not_available THEN
        RAISE EXCEPTION 'V98 requires quiescent order, inventory and shipment facts; retry after active writes finish'
            USING ERRCODE='55P03';
    END;
