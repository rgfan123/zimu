 ...
 * {@link LangChain4jRuntimeAdapter}（真实模型，04 票 Runtime Adapter），否则注册
 * {@link DefaultAgentRuntime}（fail-closed 兜底）。base-url 在 yml 中恒存在（默认空串），
 * 故用表达式按非空互斥，而非 @ConditionalOnProperty（对空值也会匹配）。
 * {@code app.message-interpreter.*} 行为不受影响。
 *
...
public class AgentRuntimeConfiguration {
    ...
    AgentRuntime langChain4jRuntimeAdapter(
            AgentModelProperties properties, AgentObservability observability) {
        return new LangChain4jRuntimeAdapter(properties, observability);
    }
