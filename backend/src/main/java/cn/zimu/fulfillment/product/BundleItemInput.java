...
public record BundleItemInput(
        @NotNull(message = "组件 SKU 不能为空") @Pattern(regexp = Patterns.IDENTIFIER, message = "组件 SKU 标识符无效") String skuId,
        @NotBlank(message = "单份用量不能为空")
                @Pattern(regexp = Patterns.POSITIVE_INTEGER_QUANTITY, message = "单份用量必须为正整数")
