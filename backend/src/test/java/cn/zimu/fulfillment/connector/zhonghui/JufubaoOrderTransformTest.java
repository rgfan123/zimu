 ...
 * orders/query 响应体）验证字段映射、收货人留空、rawSnapshot 脱敏。
 */
class JufubaoOrderTransformTest {
    ...
    @Test
    void mapsCapturedOrderToStructuredRowWithEmptyReceiverAndSanitizedSnapshot() throws Exception {
        ...
        // 收货人留空（票 15 blocker），不拒绝整单
        Receiver receiver = canonical.receiver();
        assertThat(receiver.name()).isEmpty();
        assertThat(receiver.phone()).isEmpty();
        assertThat(receiver.address()).isEmpty();
        ...
        // 非敏感字段原样保留
        assertThat(snapshot.get("order_status")).isEqualTo("NO_DELIVERY");
    }
