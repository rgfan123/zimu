...
| # | 项 | 状态 |
|---|---|---|
| 1 | 票 11 来源回填在线推送写路径（V34 是死 schema） | ✅ 已补回（`2e1b415`），见 issues/01 Resolution |
| 2 | 中汇 PMS 上传前端（后端 7 端点前端零调用） | ✅ 已补回（`0790920`），见 issues/02 Resolution |
...
~~**01 / 02 票完成前不要删这个分支。**~~ —— 两票均已完成并合入 master，
快照分支的独有内容（中汇上传前端 4 个文件）已进主干。仍建议保留一段时间：
快照里还有 JD 查询共享重构（`JdQueryPage`/`PageState`/`jdQueryCore`）等未评估内容。
