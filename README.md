...
需要客户跟进 +1 等“真实操作人”语义时，必须启用可选的逐人模式：设置 `APP_OPERATOR_HTPASSWD_B64`
（bcrypt htpasswd，用户名必须等于 `InternalOperator.wecom_userid`）和独立的 32+ 位
`APP_GATEWAY_ASSERTION_TOKEN`。Nginx 验证各自凭据后覆盖身份头，后端再独立复验 assertion token。
未启用该模式时，共享管理员账号不构成逐人归因，+1 深链会按真实 `wecom_userid` 校验并 fail closed。
