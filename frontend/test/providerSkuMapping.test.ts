...
test('京东件数换算与后端公开 int32 JSON number 契约一致', () => {
  for (const value of ['1', '999']) {
    assert.equal(JD_PIECES_PER_UNIT_PATTERN.test(value), true, value);
    assert.equal(optionalJdPiecesPerUnit(` ${value} `), Number(value));
  }

  for (const value of ['0', '0.000', '-1', '1.0001', '.5', '1.', '2.5']) {
...
test('空换算保持未配置，不暗中默认为 1 件', () => {
  ...
  assert.equal(jdPiecesPerUnitLabel('2'), '2 件');
});
