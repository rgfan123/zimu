...
function statusTagColor(status: ChannelStatus): 'success' | 'error' | 'default' {
    ...
    <Space direction="vertical" size={12} style={{ width: '100%' }}>
      {summary.hasNewOrders ? (
        ...
        <Alert
          ...
          showIcon
          message="同步未完成"
          description={`${summary.failedCount} 个渠道失败，请重试`}
        />
      ) : (
        <Alert
          type="info"
    ...
    <Card size="small" hoverable={Boolean(channel.destination)} style={{ width: 320 }}>
      <Space direction="vertical" size={6} style={{ width: '100%' }}>
        <Space style={{ width: '100%', justifyContent: 'space-between' }}>
          <Typography.Text strong>{CHANNEL_LABELS[channel.channel]}</Typography.Text>
