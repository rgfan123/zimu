/**
 * 批次快照弹窗的**取数口径**：订单实体优先，`raw_cells` 快照退居证据。
 *
 * <p><b>它修的是什么</b>（2026-08-28 生产实证）：聚福宝新单在弹窗里除渠道单号外整行破折号——
 * <pre>
 *   序号  渠道单号                 收件人  电话  收货地址  商品  件数
 *   1     m951890039794349980      —      —     —        —     —
 * </pre>
 * 而数据其实都在：`app.orders` 有收件人「丁」、电话、地址，`app.order_lines` 有商品名与数量。
 *
 * <p><b>根因不是数据缺失，是取数路径选错了源</b>：弹窗把 `raw_import_rows.raw_cells` 当唯一数据源，
 * 而那份快照是**刻意不含 PII 的**——它会被渲染进企微群卡片，所以收件人/电话/地址在快照里被掩码
 * 或剔除（同一份快照里 `supplier_name` 也是 `"京诚乾***"`）。快照不含 PII 是对的，
 * **不要去改快照，也不要动企微渲染路径**——那条边界是硬的。要改的是这个内部弹窗读哪儿。
 *
 * <p><b>三级口径，永远显式标注来源</b>：
 * <ol>
 *   <li>`ORDER`：订单实体（`GET /api/v1/orders/{id}`，经 `raw_import_rows.order_id` 关联）。
 *       内部工作台展示 PII 可以，与复核卡片同一边界。</li>
 *   <li>`SNAPSHOT`：没建单（或订单读不到）时退回快照/解析投影——聊胜于无的证据口径。</li>
 *   <li>`NONE`：两边都没有。渲染层据此显示「未建单」等**具体原因**，
 *       而不是一个让人误以为「平台没给数据」的破折号。</li>
 * </ol>
 *
 * <p><b>商品与件数必须同源</b>（既有路由测试锁定的不变量）：两者要么都来自订单行、
 * 要么都来自快照，绝不允许一个取订单、一个取快照——混口径的行对不上账。
 *
 * <p>纯函数、无 React、无 fetch，可被 node:test 直接加载（相对路径 + `.ts` 后缀，不走 `@/` 别名）。
 */

/** `presentImportRow` 对缺失值的既有占位——它不是真值，判定时必须先剥掉。 */
const EMPTY_MARK = '—';

/** 事实来自哪一层。渲染层据此决定是否标注「快照口径」以及占位文案。 */
